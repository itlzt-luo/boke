...
public class CommentService {
    ...
    public void insert(Comment comment, User commentator) {
        ...
        if (comment.getType() == CommentTypeEnum.COMMENT.getType()) {
            ...
            // 创建通知
             this.createNotify(comment, question.getCreator(), commentator.getName(), question.getTitle(), NotificationTypeEnum.REPLY_QUESTION, question.getId());
        }
    }
...
/*        if (receiver == comment.getCommentator()) {
    ...
    public List<CommentDTO> listByTargetId(Long id, CommentTypeEnum type) {
        ...
        //获取去重的评论人
        Set<Long> commenttators = comments.stream().map(comment -> comment.getCommentator()).collect(Collectors.toSet());
        List<Long> userIds = new ArrayList<>();
        userIds.addAll(commenttators);
